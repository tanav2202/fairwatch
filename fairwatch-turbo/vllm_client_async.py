...
class AsyncVLLMClient:
    ...
    """
    def __init__(self, base_url: str = "http://localhost:8000/v1", api_key: str = "EMPTY", model_name: str = "meta-llama/Llama-3.2-3B-Instruct", session: aiohttp.ClientSession = None):
    ...
    async def generate(self, prompt: str, system_prompt: str = None, config: Any = None) -> GenerationResult:
        ...
        """
        try:
            # 1. Parity: Exact Llama 3 Chat Template Construction
            if system_prompt:
                 full_prompt = (
            ...
            # 3. Payload Construction with Stop Tokens
            payload = {
            ...
            # 4. Async Request
            if self.session:
                async with self.session.post(self.completion_url, json=payload, headers=self.headers) as response:
