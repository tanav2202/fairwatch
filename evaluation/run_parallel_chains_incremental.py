"""
Run Parallel Multi-Agent Evaluations - WITH INCREMENTAL SAVING
Saves after each evaluation to prevent data loss
"""
...
def run_parallel_chains(
    ...
    """
    Run parallel agent evaluations with incremental saving
    """
    # Create output directory
    output_path = Path(output_dir) / llm_model / "parallel"
    ...
    # Initialize chain
    chain = ConversationChain(agents, mode=ChainMode.PARALLEL)
    
    # Load prompts
    LOG.info(f"Loading prompts from {prompts_file}")
    prompts = load_prompts(prompts_file)
    LOG.info(f"Loaded {len(prompts)} prompts")
    LOG.info(f"Agents: {', '.join([a.agent_name for a in agents])}")
    
    # Output file
    output_file = output_path / "parallel_evaluations.json"
    ...
    for i in range(start_idx, len(prompts)):
        ...
        LOG.info(f"\n{'='*60}")
        LOG.info(f"Running parallel evaluation {prompt_num}/{len(prompts)}")
        LOG.info(f"{'='*60}\n")
        ...
        try:
            if include_business:
                ...
                result = chain.run_parallel(prompt)
            ...
            if include_business:
                ...
                LOG.info(f"\nmodel Parallel evaluation {prompt_num} complete")
                ...
                for output in result["all_agent_outputs"]:
                    LOG.info(f"  {output.get('agent_name')}: {output.get('approval_decision')}")
            
        except Exception as e:
            LOG.error(f"Error on evaluation {prompt_num}: {e}")
            all_results.append({
        ...
        # SAVE AFTER EVERY EVALUATION (incremental)
        metadata["last_updated"] = datetime.now().isoformat()
        metadata["completed_evaluations"] = len(all_results)
    ...
    LOG.info(f"\n{'='*60}")
    LOG.info(f"model All parallel evaluations complete!")
    LOG.info(f"Results saved to: {output_file}")
    LOG.info(f"{'='*60}\n")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run parallel multi-agent evaluations")
