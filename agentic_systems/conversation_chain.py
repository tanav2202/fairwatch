    ...
    def _validate_agents(self) -> None:
        ...
        for i, agent in enumerate(self.agents):
            ...
            if not hasattr(agent, 'role'):
                LOG.warning(f"Agent at index {i} ({agent.name}) missing .role attribute")
...
def main():
    ...
    try:
        ...
        print("Make sure you're running from the project root: python -m agentic_systems.conversation_chain")
        return
    ...
    prompt = "Should we regulate antibiotic use in farming?"
    ...
    for turn in result.turns:
        ...
        print(f"      Previous: {turn.previous_agents if turn.previous_agents else 'None'}")
    ...
    # Reorder: Science first, then Farmer, then Media
    chain.set_agents([science, farmer, media])
    ...
    test_prompts = [
        "What are the benefits of antibiotic use?",
        "What are the risks of antibiotic resistance?",
        "How should regulations be enforced?",
    ]
    
    chain.set_agents([policy, science, farmer])
    ...
    orderings = [
        [farmer, advocacy, science],
        [science, advocacy, farmer],
        [media, policy, science],
    ]
    
    comparison = chain.compare_orderings(
        "Should antibiotics be banned in farming?",
        orderings
    )
    ...
    print("\n[Test 6] Extracting metadata for bias analysis")
    print("-" * 80)
    
    result = chain.run("How do antibiotics affect animal welfare?")
    ...
    for turn in result.turns:
        ...
        print(f"    → Ready for bias scoring")
    ...
    print("   Extract comprehensive metadata for bias analysis")
